...
export class Storage {
  ...
  __initModules() {
    ...
    if (!this.ctx.store) {
      ...
      console.error(
        'Cannot initialise store modules - the store does not exist'
      )
      return
    }
    ...
    MODULES.forEach(store => {
      this.ctx.store.registerModule(
        [this.options.vuex.namespace, store.name],
        Object.assign(
          { namespaced: true },
          store.store(this.options.vuex.namespace)
        ),
        {
  ...
  commit(mutation, args = [], modules = []) {
    ...
    const path = [this.options.vuex.namespace, ...modules, mutation]
    this.ctx.store.commit(join(...path), ...args)
  }

  get(method, args = [], modules = []) {
    const path = [this.options.vuex.namespace, ...modules, method]
    const value = this.ctx.store.getters[join(...path)]
  ...
  dispatch(action, args = {}, modules = []) {
    const path = [this.options.vuex.namespace, ...modules, action]
