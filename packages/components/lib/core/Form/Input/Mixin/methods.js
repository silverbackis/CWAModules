...
export default {
  methods: {
    ...mapMutations({
      ...
      setInputLastValidationValue:
        'bwstarter/_forms/setInputLastValidationValue'
    }),
    ...mapActions({
    ...
    async validate() {
      ...
      const postObj = this.inputSubmitData
      const simulatedInputNames = []
      if (this.isRepeated) {
        for (const child of this.parents[0].children) {
          const childName = child.vars.full_name
          if (childName !== this.inputName) {
            const repeatedFieldPostObj = this.getInputSubmitData(
              this.extendInputId({}, childName)
            )
      ...
      try {
        ...
        this.applyFormResultValidation({
          formId: this.formId,
          formData: data.form,
          simulatedInputNames
        })
