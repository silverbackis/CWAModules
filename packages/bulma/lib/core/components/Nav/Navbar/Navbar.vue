<template>
  ...
  >
    ...
    <div class="navbar-menu" :class="{ 'is-active': isActive }">
      <div v-if="$slots['navbar-start'] || (!itemsAtEnd && childComponents.length)" class="navbar-start">
        <bulma-navbar-item v-if="(!itemsAtEnd && childComponents.length)"
        ...
        <div v-if="$slots['navbar-end'] || (itemsAtEnd && childComponents.length)" class="navbar-end">
          <bulma-navbar-item v-if="(itemsAtEnd && childComponents.length)"
                             v-for="(component, index) in childComponents[0]"
                             :component="component"
                             :key="index"
